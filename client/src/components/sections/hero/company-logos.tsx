        ...
        <div className="flex animate-scroll">
          ...
          <ul className="flex shrink-0">
            {companyLogos.map((logo, index) => (
              <li key={index} className="flex h-[8.5rem] w-[200px] flex-shrink-0 items-center justify-center px-8">
                <Image src={logo} width={134} height={28} alt={`Company logo ${index + 1}`} />
              </li>
            ))}
          </ul>
          ...
          <ul className="flex shrink-0">
            {companyLogos.map((logo, index) => (
              <li key={`duplicate-${index}`} className="flex h-[8.5rem] w-[200px] flex-shrink-0 items-center justify-center px-8">
                <Image src={logo} width={134} height={28} alt={`Company logo ${index + 1}`} />
              </li>
            ))}
          </ul>
